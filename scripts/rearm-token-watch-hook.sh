...
3. Weekly digest SURFACING (Monday 08:23 local). If a job with this weekly-surfacing prompt is present, skip. Else arm: CronCreate(cron="23 8 * * 1", recurring=true) with a prompt that wakes the peer at claude-hive stable_id 004e35052d8b — spawning its session first if it is not running, path per registry.yaml — and hands it the goal: surface the digest its own pipeline already generated this morning to Bryan, on whichever surface he is active on (Claude Remote before CLI). The peer owns how; do not prescribe commands and do not ask it for status.
   ...
   Do NOT ask it to run a refresh. As of 2026-08-11 that peer owns a launchd agent that runs the whole pipeline at 07:00 daily, so the data half of this job is handled at the OS layer — which is strictly more durable than a session-scoped cron, since it survives respawns and reboots. Asking for a refresh here would re-run a scrape and a paid aggregator pull on top of the 07:00 run and hand Bryan a second digest an hour after the first. This job is read-and-relay only.
EOF
