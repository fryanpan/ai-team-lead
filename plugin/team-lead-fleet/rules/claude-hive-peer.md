...
When this project is running as a **peer in a claude-hive network** (the team-lead session in `ai-team-lead` is a separate peer; other project peers may also be running), follow this protocol so coordination across sessions is consistent.
...
- **Status updates: 3–5 per task max.** Start, blockers, PR open, merge, done. Don't flood the team-lead with play-by-play.
- The user reads the team-lead, not individual peer stdouts. If you need human input, route it via the team-lead.
