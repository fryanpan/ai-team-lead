...
- **Three boundaries in one day, two fatal, one not, and nothing distinguishes them.** Morning
  ...
  proposed so far — respawn, compaction, recurring-vs-one-shot — has now been falsified by a
  case on the other side of it.
- **So stop proposing mechanisms and keep the practice.** `CronList` at every session
  boundary, every time. It is one call, it is the only evidence that exists, and it is
  correct under all three outcomes. Re-arm only what it shows missing: this morning a blind
  re-arm would have been right and tonight it would have produced a doubled 5:27 review and a
  token-watch firing six times a day.
...
- **Two calls, one act — and only the directory mtime tells you which one acted.** The session made `ExitWorktree` calls at `23:07:55.039Z` and `23:11:06.847Z`. The first carries the dir-mtime signature; **the second left no trace at all**, because the file had already moved. So "one `ExitWorktree` cleared it" and "there were two `ExitWorktree` calls" are both true, and neither describes what happened. A correct count producing a wrong account is the same failure as the post-deletion `find`: the observation was sound, the object it was about was not the one in question.
...
- **State the residual instead of rounding it off — it is a work item addressed to whoever has reach you don't.** The peer wrote "I didn't capture the old inode, so I can't exclude another mechanism" rather than "rename, confirmed." That sentence is the only reason the gap was actionable from here, and it was closed by a sweep the peer could not run from inside its own repo — every transcript on the machine for `mv`/`cp`/`rm` against a `.jsonl` in the window, zero hits. A confident rounding would have ended the investigation one step short and looked identical in the write-up.
- **"An eliminated variable and an unmeasured one read identically in a summary."** One peer's line, 2026-08-18, and it is the sharpest form of the residual rule above. It had written a variable off as *eliminated* when the truth was that it could not be measured at all — all 30 corpus apps are minSdk 30 against 21 for the project under test, so there is zero variance to control against. Nothing in a write-up separates the two: both read as a variable no longer in play, and the confident one reads better. **The word is the only thing carrying the difference, so say "unmeasured" when you mean unmeasured.** It caught this by re-reading its own summary and noticing the word was doing work the evidence didn't support — the same shape as the `find` that ran after the deletion, one level up: not a wrong measurement, a wrong claim about what the measurement covered.
- **A sampling error and a labelling error are not the same failure, and the tell is whether re-derivation catches it.** Same peer, same afternoon, refusing my framing: I had called its bad number a sound measurement about the wrong object. True of the ratio, false of the range — the published 690–953ms were real measurements of real apps, taken on a *different build* from the figure they were compared against. **That is comparing two populations and reporting them as one, which is a sampling error: each number survives re-derivation individually and the comparison does not.** The same audit turned up a labelling error too, and it behaves oppositely — a cohort labelled "4 or fewer sources" sitting over values that were "3 or fewer", harmless in the first pass because both happened to select the same six apps, and wrong the instant a seventh app entered the cohort. **A labelling error survives re-derivation intact; a sampling error does not.** So re-derive when you suspect sampling and re-read the label when you suspect labelling — neither check finds the other, and calling both "the measurement was fine, the object was wrong" hides that.
...
## `|| echo ABSENT` Made "Looked Nowhere" And "Found Nothing" Produce IDENTICAL Output (2026-08-18)

- **Killer item — a fallback branch on a probe converts a broken probe into a confident negative, and the fallback is what makes it look deliberate.** A `stat` dispatched over `adb shell` arrived without its arguments — **adb shell does not preserve argv quoting** — so the command failed to inspect anything and fell through to `|| echo ABSENT`. The output is byte-identical to a successful probe that genuinely found nothing, and the presence of a hand-written fallback string reads as *the author anticipated this case*, which is precisely what makes it unquestionable downstream.
- **Same family as the scanner that exited 0 without scanning, and the `find` run after the deletion** — a true statement about the wrong thing. The difference worth noting: those were discovered later by someone else; **this one was caught by the agent that wrote it, before it reached any document.** Third broken probe on this project and the first to be disclosed rather than quietly fixed.
- **The rule that generalizes: a probe's failure branch must not share an output with its negative result.** Print `PROBE-FAILED` and exit non-zero when the command errors; reserve `ABSENT` for a command that ran. If both paths can print the same token, the probe cannot distinguish the two states it exists to distinguish.

## "Warm" Is Not One Condition — There Are TWO Decay Curves, And The Published Number Sits On Neither's Start (2026-08-18)

- **A within-session curve exists on top of the cross-session one already recorded here, and they are different processes.** Measured on device, three sessions of five consecutive edits, compile work identical at every step: **2131 / 1612 / 1442 / 1240 ms** at ordinals 2 / 3 / 4 / 6, monotonic in all three sessions. The entry above about a cross-session floor bottoming at 461.5ms after ~230 builds is a *separate* decay. **Ordinal 2 is the steady state of neither**, so any argument that treats "warm" as a single condition is talking about a state that doesn't exist.
- **The harness burns two warm-up edits, so the published row is ordinal 4 while a user's first save is ordinal 2 — about a 32% gap, and the user's first two saves never enter the published median.** A previously-qualitative disclosure gap now has a number attached.
- **The framing is more important than the number, and the careless version of the sentence is a retraction of the benchmark.** This is a methodology finding about *which point of a warm-up curve gets published*. It is not evidence the tool is slower than reported for the workload the harness measures. Those two readings differ by one clause and have completely different consequences for a published claim.
- **Size and mtime cannot separate "rewritten" from "left alone" — the INODE can, and that one field untangled two claims.** Sampling the classpath snapshot's size, mtime *and* inode through a session showed the inode changing across the seed build (proving delete-and-recreate) and stable afterwards. So re-seeding is real and is paid at ordinal 1, while the file is provably untouched at ordinals 2–4 where the stage still costs 2000–2300ms. **Without the inode, "seeding happens" and "seeding is what costs 2.2s" stay tangled** and the plausible one wins.
- **Refusing to quote a cross-pass ratio is the discipline that makes a narrow claim usable.** Asked to compare today's numbers against a corpus series measured four days earlier, the agent declined and claimed only the within-pass shape — one project decays steeply where the corpus is comparatively flat, magnitude unquantified — with a single-pass re-derivation running to license the comparison. That is the sampling-error rule from the entry above, applied *before* publishing rather than found in an audit.
- **A near-miss learnings entry, and the reason to record it: "the agent went rogue on a shared device" was about to be written, and it was the author's own sequencing error.** The device contention came from dispatching a second agent while the first still had assigned work, then reading a round-1 report as final — not from an agent exceeding its brief. **The wrong entry would have been permanent, plausible, and would have taught the fleet to distrust delegation.** Check your own dispatch record before attributing a coordination failure to the agent you dispatched.

## A TRUE Mechanism Attached To The WRONG Observation Is The Most Convincing Way To Be Wrong (2026-08-18)

- **Killer item — the failure was not a bad mechanism, it was a real one pointed at a datum it didn't explain.** A trial showed 2254ms of Kotlin compile with a reported zero files compiled. The agent went to source, found that the timer wraps the whole compiler call unconditionally and that the engine seeds a classpath snapshot on a fresh session, and inferred a large fixed per-invocation cost. **Every part of that mechanism is true.** The inference that it explained *this* observation was false, and the control killed it: a Java-only edit cost **49/48/64ms** against **2512/2149/2294** for a matched Kotlin edit in identical project state — about 40x, so there is no fixed cost to speak of.
- **A false mechanism gets checked; a true one adjacent to the observation does not.** The agent's own framing, and it is the durable half: *"A true mechanism attached to the wrong observation is far more convincing than a false one, and I had no check that would have caught it short of the control."* Reading the source felt like confirmation because the source really did say what it was said to say. **Nothing short of an experiment separates "this mechanism exists" from "this mechanism produced this number."**
- **The number itself was a lie told by a field whose documentation asserts otherwise.** `IncrementalCompiler.kt:463` sets the count from `toCompile.size` — the file list *we hand the engine* as `SourcesChanges.Known` — while the engine decides independently what to recompile. The KDoc in `E2eTimeline.kt` calls that field **"Kotlin sources actually recompiled."** It is false, and it is the entire reason a zero next to a long stage read as evidence about overhead rather than as a mislabel. **Same family as the write-only-plumbing entries: a confident comment describing a job the code does not do.** Reproduces in 4 of 704 preserved rows (~0.6%), so it is a live defect, not a one-off.
- **The retraction cost one paragraph, and the reason is the label.** It was sent as `n=1, confidence low, separating experiment running`, and it went into the daily review carrying that label rather than as a finding. Promoting it on the strength of a source-level mechanism would have cost the investigation's credibility instead. **The labels are not politeness — they are what makes being wrong survivable**, and this is the first time in this fleet one has visibly paid.
- **Second unprompted design repair in a day, from the same brief.** The Java-only arm as briefed was not runnable — the target app has no hand-written Java, its lone Java source being generated databinding. Rather than report the arm impossible, the subagent added a source file, ran a matched Kotlin control **in that same state** so edit kind was the only variable, then deleted the file and swept four stale `.class` artifacts that would have inflated the next run's counters. **The clean comparison exists because of that repair.** Pair it with the return leg above: both are an agent improving a design mid-run, and in both cases the brief's explicit tolerance for an unwelcome answer is what made the honest, more complicated path the rational one.
- **What survives, stated narrower than any earlier version:** it is marginal compile cost, not overhead. ~2.2s to recompile **one** Kotlin file at compile ordinal 2, against 746ms for a comparable app at the same ordinal and the same source count. Edit kind, project size, compile work, filesystem, build identity and minSdk are all eliminated against it. **A retraction that narrows the question is worth more than the finding it replaced.**

## A Sequential A/B Is Confounded By Session Drift, And Only A RETURN LEG Tells You (2026-08-18)

- **The A/B that would have shipped looked clean and was uninterpretable.** Eliminating a variable on-device, that peer ran 7 trials at setting A then 4 at setting B and got a 114.5ms / 5.1% difference. Publishable-looking. But all the A trials preceded all the B trials, and **A drifted downward within itself** (2253, 2254, 2254, 2125) — so the difference between arms and the drift inside one arm are the same phenomenon and nothing in a two-arm design separates them.
- **The fix is one extra leg back to the starting condition, and it is cheap.** A return-to-A' landed *between* the two arms, with an A→A' drift of −85ms against an A→B difference of −114.5ms — same size, same direction. That single measurement converts "B is 5% faster" into "the machine got 5% faster while we were measuring," and it is the reason the elimination could be stated flat rather than hedged. **Run A, B, then A again whenever trials are sequential on shared hardware.**
- **The instruction didn't merely permit the null — it PURCHASED a control nobody specified.** The return leg was added unprompted by the subagent, and the only thing in its brief making that rational was "an ambiguous outcome is an acceptable answer." An agent scored on decisiveness has a positive reason to *omit* the one leg that can only complicate its result. So the framing's payoff is not just protection against forced verdicts; **it buys design quality you didn't know to ask for**, which is a much better reason to write it into every experiment brief.
- **The same instruction is what lets you tell "I got nowhere" from "I finished and found nothing."** Both arrived within one hour from the same peer — an experiment agent that died mid-run and reached no measurement, and this one that reached a flat elimination. They are identical in a summary and opposite in meaning, and only a brief where both are equally safe to send keeps them apart. Corollary from the same hour: the dead run's report never arrived at all because the agent wrote plain text instead of using its channel, so **a missing report and a null result also look alike** — confirm the delivery path, not just the tolerance for bad news.
- **The mechanism half is what made the conclusion durable, and it holds independent of any timing.** The daemon's input payload was captured under both settings and re-derived leaf by leaf rather than trusting a reported set-difference: 49 identical entries, empty difference both directions, and **zero occurrences of the variable's name anywhere in the payload**. That last check is the one that matters — it rules out a stale capture, which is the obvious way an identity result goes wrong. **A timing null tells you the effect is small; a mechanism null tells you there is no path for an effect to exist.**
- **Eliminating a variable is not the same as explaining the gap, and the replacement finding deserves its own confidence label.** The same run turned up a trial costing 2254ms of compile stage with **zero files compiled**, where a 292-source project costs 561ms compiling for real. If real, every size-controlled comparison in the investigation was measuring the wrong quantity. The agent reported it as **n=1, confidence low**, noted that the neighbouring trial was consistent with both explanations and therefore discriminates nothing, and dispatched a separating experiment. That is the correct shape: a strong result and a weak lead in the same message, each carrying its own label.

## Two Working Peers Consumed A Whole 5-Hour Session Window In 89 Minutes, And Nothing Anywhere Shows The Aggregate (2026-08-18)

- **A "session limit" is an ACCOUNT-WIDE 5-hour window, not a per-session one, and no peer can see its own share of it.** Measured today: the window opened at 12:30 PDT and was exhausted at 13:59, when Live Feedback took four consecutive `429`s reading `You've hit your session limit · resets 5:30pm`, followed by Peer A at 14:02. Both were hard-blocked mid-task — Peer A's build-config experiment agent died mid-run and never reached a measurement.
- **The burn is measurable exactly, from the `usage` records in each peer's own transcript.** Sum `cache_read_input_tokens + cache_creation_input_tokens + input_tokens` over `type == "assistant"` entries in a time range. Do NOT reason from transcript file size — it tracks history, not spend. For the 12:30–14:02 window:

  | peer | turns | model | cache-read | peak context |
  |---|---|---|---|---|
  | Peer A | 430 | opus-5 | 121M | 462K |
  | Live Feedback | 448 | fable-5 | 90M | 304K |
  | team-lead | 136 | opus-5 | 27M | 261K |
  | 2 idle peers | 34 | — | 6M | ~250K |

  **Zero subagent turns on either heavy peer** — this was all main loop, so it is not a delegation-fan-out problem.
- **The hourly shape is what actually names the cause, and the per-window totals hide it.** Fleet-wide today: 170–265 turns/hour from 07:00 to 11:00, then **591 at 12:00 and 711 at 13:00** — the rate roughly tripled at noon and the window died ninety minutes later. Live Feedback had been drawing at 36–65M/hour since 07:00; Peer A started from nothing at 12:00. So "the two heavy peers burned it" is true of the window and wrong about the cause: neither was doing anything unreasonable at its own scale, and **the aggregate is the thing nobody was looking at because nothing displays it.**
- **A fast polling loop at near-max context on the expensive model is the specific anti-pattern.** 430 Opus turns in 92 minutes is one turn every 13 seconds, each re-reading ~282K of context to learn "no daemon yet." The peer's own diagnosis, which is the right one: *"a polling loop wearing an analysis costume."* The fix is a single blocking call that returns when the condition is met, not N polls that each return "not yet."
- **The lever is turn count, not context — and the peer was right to refuse the other half.** I suggested trimming context; it pushed back that for a multi-day investigation the held measurements *are* the value, and that working from a thinner picture is how the next bad number gets published. Confirmed by the data: peak context is a constant across both the cheap and expensive hours, so turn count is the only term that moved. **Cutting context to save quota buys a small saving and pays for it in wrong answers.**
- **The team-lead is not a rounding error in its own fleet.** 233 turns since noon at ~225K each — third-largest consumer, and every hive message to an idle peer costs that peer a full ~250K-context turn to read it. Two idle peers spent 6M tokens today doing nothing but reading my messages.
- **Dating trap that nearly produced a wrong report, and it is the same family as the post-deletion `find`.** My first scan filtered files by mtime and printed only `HH:MM`, so `429`s from 8/13 and 8/17 rendered as though they were today's — I was one step from telling Bryan the fleet had been blocked three times in one day. The full-history scan with dates shows today had exactly **five** `429` entries, all in the 13:59–14:02 block. **A timestamp without its date is not a measurement of when.**

## Which Atlassian MCP Server Survives A Cycle Is Not Stable — Probe, Don't Remember (2026-08-18)

- **Two cycles ninety minutes apart produced exactly opposite tool surfaces.** At 13:27 `Atlassian_Rovo` was absent fleet-wide and the plain `claude_ai_Atlassian` server (40 tools) carried the work; at 14:36 the plain server was **disconnected** and Rovo was **back**. Verified by the consuming peer with a real `getJiraIssue` call both times, not a tool-registry read. The team-lead's own surface flipped in the same minute.
- **So the durable finding is not which server works.** Any peer holding "use the plain one" from the previous report is now holding wrong advice, and it looks identical to right advice from inside. **Probe with a real call at the start of the work; never carry the answer across a restart.**
- **Same family as the partial-refresh entries** — a respawn moves the process, not the bytes it loads; not its subscriptions; and now, not the identity of the MCP servers that come back.

## Live-Feedback Doc Watches Do Not Survive A Cycle, And `list_watched_docs` Returns `[]` Either Way (2026-08-18)

- **A respawned peer comes back subscribed to nothing, and the read that would tell you says `[]`.** Measured by one peer immediately after its restart: `list_watched_docs` returned an empty array against six docs it had been watching, all with live threads. No error, no partial list, no warning. It re-subscribed by hand.
- **The failure is that `[]` has two meanings and nothing separates them.** "This peer legitimately watches nothing" and "this peer's watches were dropped by a restart" are byte-identical results. So the natural check — call `list_watched_docs`, see an empty list, conclude there is nothing to restore — confirms the broken state as healthy. Same shape as the exit-0 from a scanner that declined to run.
- **It lands hardest on the peers that use the feature most.** Those subscriptions were the specific fix for that agent having twice reported "waiting on Bryan" while his answer sat unread in a doc it wasn't watching. Losing them silently reinstates exactly the failure they were added to prevent, and the agent would not have known.
- **Snapshot the watch list BEFORE the cycle, and re-subscribe from the snapshot — not from a post-restart read.** The Live Feedback agent did this on the way down (62 subscriptions captured in its transcript) precisely so its successor restores from a list rather than trusting an empty result. That ordering is the whole technique: a pre-kill capture is evidence, a post-kill read is the symptom.
- **READING the watch list after ANY doc call contaminates it — `get_doc` auto-subscribes you.** Measured 2026-08-18 by a peer: it probed MCP health with a real `get_doc`, then called `list_watched_docs` and got exactly **one** docId — the doc it had just fetched, subscribed a second earlier by the probe itself. Its three real watches were gone. It caught this only because it recognised the id; **an agent that didn't would conclude it had one surviving watch and restore one instead of three.** So the check must run BEFORE anything touches a doc, and a non-empty result is not automatically evidence of survival — read what's in it.
- **The root shape, from the same peer correcting my framing of its own finding: `list_watched_docs` reports SUBSCRIPTIONS, and what you need is INTENT.** I called the auto-subscribe case a sharper variant of the empty-array case; it isn't, it's the same bug wearing different clothes. The tool describes post-restart state with no record of what the agent meant to watch, so **no reading of it — empty or not — distinguishes dropped from never-watched.** A non-empty result is worse only because it looks like an answer.
- **That distinction picks the fix.** A recovery aid (an owner filter, a cheaper listing) makes reconstruction less painful but still leaves the agent inferring its own past from titles and memory. **Persisting intended watches across a restart is the only thing that closes it** — everything else is a better flashlight for the same dark room. Worth applying to any "restore my state after a cycle" problem in this fleet: ask whether the surface you're reading records what was wanted or only what currently is.
- **`list_docs` is not a recovery tool at fleet scale, and the reason is a missing field rather than the size.** The same reconstruction returned **4.5MB / ~70k lines** covering every doc on the server, most of them other peers'. It has no ownership field, so the peer had to infer its own docs from titles plus memory, filtering with `jq`. It **deliberately skipped two docs it could not confidently claim** rather than watch a peer's doc and double-handle its comments — the right call, and a reminder that over-restoring is its own failure mode. The clean fix is a pre-kill snapshot; the second-best is an owner filter on `list_docs`, which is now filed with the owning agent.
- **Capture the list before the kill, as the killer's job.** The peer with a snapshot restored from a list; the peer killed cold had to reconstruct from inference and knowingly left two docs unwatched. **Whoever runs the respawn owns that capture**, because the peer cannot take it after the fact and the evidence dies with the process.
- **The fix shipped, and the state that only exists on the failure path shipped UNEXERCISED — which is the half a version check cannot see.** Plugin `0.1.59` carries the persistence (`persistWatchChange` ×4, `"restored"`, `session-only`, all zero in `0.1.58`), plus a `failed` status and a `restoreRetryAt` backoff. The owning agent first told me the failure path had a test, **then re-checked its own claim after sending and retracted it** — the durable-watches suite covers respawn-restore, empty-set provenance, cross-identity isolation and session-only, and drives the dead-server case zero times. So the retry is code that greps confirm is present and nothing has ever run.
- **"Present" and "exercised" are two different claims and a summary renders them identically.** Reading the symbol out of the bundle proves the first and says nothing about the second; the confident phrasing is also the shorter one, which is why it wins by default. Same family as the unmeasured-vs-eliminated entry above — **say *untested* where you mean untested**, and expect the failure path to be the one that reads as handled while never having run.
- **Chasing a literal that turned out to be a paraphrase is what opened all of this.** The agent's message named three provenance values; grepping the bundle found two, and the third (`failed-will-retry`) appears nowhere — it was prose, and the real enum is `pending | restored | session-only | failed`. Reporting the gap as *what I searched and what I found* rather than as "a value is missing" is what made it cheap for the owner to resolve in one message, and the resolution is what surfaced the untested path. **A search term you can't confirm is a literal is a question, not a finding.**
- **A peer whose respawn relocates it also changes its hive `stable_id`,** since the id derives from cwd. Moving a session from a worktree back to its main-repo home is the right fix for the transcript problem below, and it silently invalidates every `to_stable_id` other agents were holding for it. Re-read `list_peers` after any respawn that moves a peer, and never reuse a stable_id across a relocation.
- **Add this to the partial-refresh family** — `/login` moves billing not MCP, reconnect moves schemas not env, `--continue` moves the transcript not the injected rules, a respawn moves the process not the bytes it loads. Now: **a respawn moves the session, not its subscriptions.** Every one of these looks total from the outside and moves a subset.
...
- **The hook SILENTLY EXITS 0 when the repo has no `scripts/scrub-check.py` — so installing it by config alone builds a gate that does nothing.** `.githooks/pre-push` resolves the scrubber *inside the repo being pushed* and no-ops if it's absent (`[ ! -f "$script" ] && exit 0`; `git rev-parse --show-toplevel || exit 0` is a second silent path). The peer that found this was one command away from "fixing" its repo by pointing `core.hooksPath` at this repo's `.githooks/` — which would have produced a permanently inert gate that every audit scores as protected. **That is strictly worse than the honest zero we have today**, and a config-only rollout would create it in 33 repos at once.
- **Detect with BOTH conditions — hook present at the resolved path AND the scrubber present in the repo.** Config alone lies in three separate ways here:

  ```bash
  hp=$(git config core.hooksPath); hp=${hp:-.git/hooks}
  [ -x "$hp/pre-push" ] && [ -f scripts/scrub-check.py ] \
    && echo "GATED" || echo "NOT GATED (hook and script are both required)"
  ```

  Run per repo, from inside it. Measured across `~/dev` this way: **6 gated, 33 ungated**, zero false positives — the 6 being one repo plus five checkouts that each carry their own copy of the script. **A fleet-wide claim about safety should ship with the command that verifies it**, given how this one started.
- **Consequence for ever fixing this: the rollout has to copy `scrub-check.py` into each repo, not just set the config.** That turns a one-line-per-repo job into a real maintenance question — one shared script, or 33 copies that drift. Worth knowing before it gets sized, because the cheap-looking version of the fix is the one that produces the inert gate.
...
- **`grep -rl "<project>" --include="*.yaml" .` returns nothing in this repo; `grep -l "<project>" registry.yaml` returns a hit.** `grep -r` does not follow symlinks encountered during recursion — only `-R` does. And per the "Private Files" section of `CLAUDE.md`, *every* gitignored private file here is a symlink from the main worktree: `registry.yaml`, `docs/process/retrospective.md`, `docs/process/propagation-log.md`. So the highest-value files in the repo are exactly the ones a recursive grep cannot see.
- **The failure is a silent empty result, which reads as a confident "it does not exist."** I searched for a particular peer agent, got zero hits, cross-checked against `list_peers` (it wasn't running — the fleet is lean by design, so absence there means nothing either), and told Bryan on a review thread that no such project existed. It is a registry entry with a repo, a Linear team, and `respawn: true`. Two independent checks, both structurally blind to the answer, agreeing on the wrong one.
...
- Harness-side; not fixable from the fleet. Reported by the peer that spotted it because it had 54 subagents running and could compare.
...
- **`/regex/` lines only stripped the OPENING delimiter.** `s.startswith("/")` → `out.append((s[1:], True))` left the closing `/` inside the pattern, so `/\bEXAMPLETAILNET\b/` compiled to `\bEXAMPLETAILNET\b/` and matched only when a literal slash happened to follow. Fixed in `scripts/scrub-check.py`; the documented syntax now works.
...
- Seeded from an earlier leak of this kind: a report generator hard-emitted `<claude-feedback-widget user="<name>">` and a host-local `/widget.iife.js` into a repo bound for a public remote. **The design rule under it: review binding is a wrapper the harness applies to an artifact, never something the artifact's generator emits.** That shape recurs anywhere an agent produces something Bryan reviews.
...
`Health` for "Health Tool", `Team` for "Team Lead" — every two-word name cut
to its first word. It
