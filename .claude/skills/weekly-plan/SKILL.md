...
| Block | Format | Notes |
|------|--------|---------|
| Title | `### N. <Value-forward outcome> (~Xh) <tag>` | Names the outcome **AND why it matters** — payoff, deadline, or what it unblocks. **The estimate lives in the title** — there is no separate estimate line. e.g. `### 1. ClientOrg Project Beta ready for team review so it lands in CoGo before the HOPE talk (~4–5h) ✅` |
...
9. **Break committed goals into Asana tasks — the week's primary task surface.**
   ...
   - **Leave alone:** family/others' tasks (Good Government project, Colleague A / Colleague B) and the user's Medical self-care items.
