...
- **This diagnosis was revised three times in one day; read the failure mode, not just the outcome.** The morning measurements showed launchd-spawned processes *hanging* on Data; after a reboot at 16:43 the same controls return a clean `Operation not permitted`, and a boot-disk binary succeeds. A hang and an EPERM are different states, so the earlier readings were probably correct when taken. **Treat an FDA grant as something that can silently lapse** — this one appears to have, and a reboot restored it. Before calling any boot-disk migration done, confirm the grant survives a reboot.
