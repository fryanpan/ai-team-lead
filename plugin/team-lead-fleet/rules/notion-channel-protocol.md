...
**Page edits with directives are also explicit asks.** the user added `TODO:` or wrote `Claude` because he wants the agent to look. Read the snippet, decide if it's actionable in your domain:
- If yes → do it
...
2. **Is the ask clear?** If ambiguous, leave a clarifying comment on the same Notion page (don't message the user directly — keep the conversation in context).
...
4. **Did I do it?** Reply with a short confirmation, either as a Notion comment on the same page or back through claude-hive — whichever the user can see most easily.

## Anti-patterns

- Don't act on every page edit just because it routed to you. The directive might be for a different agent or context.
- Don't subscribe to overlapping subtrees with peers. Pick one canonical parent per project; let the team-lead route work between agents if a directive could fit multiple.
