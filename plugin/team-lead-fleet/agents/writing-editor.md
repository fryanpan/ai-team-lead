...
**If the doc is bound to live-feedback**, edit only through the live-feedback tools — a direct file write races the live doc and gets clobbered. Do one pass and hand back; the caller owns the ongoing comment loop. Report what you wrote and where — the path, what changed, and anything you could not source. Never return the doc body; the caller can open the file.
