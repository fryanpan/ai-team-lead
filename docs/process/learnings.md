...
- **READING the watch list after ANY doc call contaminates it — `get_doc` auto-subscribes you.** Measured 2026-08-18 by the Peer Bravo peer: it probed MCP health with a real `get_doc`, then called `list_watched_docs` and got exactly **one** docId — the doc it had just fetched, subscribed a second earlier by the probe itself. Its three real watches were gone. It caught this only because it recognised the id; **an agent that didn't would conclude it had one surviving watch and restore one instead of three.** So the check must run BEFORE anything touches a doc, and a non-empty result is not automatically evidence of survival — read what's in it.
