...
Comments and pages created via the Notion MCP currently appear as the user (the integration uses his personal auth). Until per-agent Notion integrations are set up, **prepend every comment and every page you create with `**From: <Your Agent Name>**` on its own line**, then a blank line, then your content.

Use the friendly name from `registry.yaml`'s `session_name` field (e.g., "Blog Assistant", "Octoturtle Assistant", "Bike Map", "Team Lead"). Don't use technical IDs.
...
- You're updating a page the user explicitly asked you to maintain (e.g., a status doc you own)
